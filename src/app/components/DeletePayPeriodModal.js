...
export default function DeletePayPeriodModal({ isOpen, onClose, period }) {
  ...
  async function handleDelete() {
    ...
    const result = await deletePayPeriod(period.id)
        ...
        <div className="px-6 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-gray-900">Delete Pay Date</h3>
              <p className="mt-2 text-sm text-gray-600">
                Are you sure you want to delete the pay period from <span className="font-semibold">{formatDate(period.start_date)}</span> to <span className="font-semibold">{formatDate(period.end_date)}</span>? This action cannot be undone.
              </p>
            </div>
          </div>
