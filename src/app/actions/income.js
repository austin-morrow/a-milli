...
export async function createIncome(formData) {
  ...
  if (incomeError) {
    return { error: 'Failed to create income: ' + incomeError.message }
  }
  ...
  revalidatePath('/spending/income')
  revalidatePath('/accounts')
  return { success: true }
}

export async function updateIncome(incomeId, formData) {
  const supabase = await createClient()
  ...
  if (incomeError) {
    return { error: 'Failed to update income: ' + incomeError.message }
  }
  ...
  if (oldIncome?.received_amount) {
    ...
    if (oldDate <= today) {
      ...
      if (oldIncome.transaction_id) {
        ...
        // Clear the transaction_id from income
        await supabase
          .from('income')
...
export async function deleteIncome(incomeId) {
  ...
  if (incomeError) {
    return { error: 'Failed to delete income: ' + incomeError.message }
  }
